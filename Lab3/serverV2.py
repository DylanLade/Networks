...
def new_client(client_socket, client_address):
	...
	# Check sent message for login command
		if parse[0] == "login":
			try:
				if current_user != "":
					client_socket.send(bytes("There is a user already signed in please logout to change users", "utf-8"))
					continue
				else:
	...
	# Check sent message for newuser command
		if parse[0] == "newuser":
			try:
				if current_user != "":
					client_socket.send(bytes("There is a user already signed in please sign out to create new user", "utf-8"))
					continue
...
while True:
	client_socket, client_address = server.accept()
	print(F"Accepted connection from {client_address[0]}:{client_address[1]}")
	thread.start_new_thread(new_client,(client_socket, client_address))
